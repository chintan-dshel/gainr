...
## [4.0.0] — 2026-04-25 — Design Refresh & Coach Intelligence

### Summary
Major design overhaul and five new features shipped in a single release. The app's visual layer was rebuilt from scratch using a perceptually-uniform OKLCH color system and a new three-font typographic stack. Five critique-driven features were added: a first-run experience, a dedicated Coach tab, interactive chart tooltips with date range filtering, a versioned AI coaching prompt, and visual polish across the character card and page layout. The app grows from ~108KB to ~251KB reflecting the additional engine code.

### Changed — Design System
- **Color system**: migrated from ad-hoc `rgba()` values to OKLCH perceptual color space (`oklch(15% 0.012 260)` base). Legacy aliases (`--gold`, `--red`, `--green`) preserved — zero breakage to existing code
- **Typography**: new three-font stack — Archivo Narrow (display/headings, `--fd`), Inter (body, `--fb`), JetBrains Mono (numbers/code, `--fm`)
- **Character card**: quieter treatment — hardcoded blue gradient replaced with OKLCH token, glow opacity reduced, XP bar thinned from 6px to 4px
- **Page titles**: reduced from 42px to 32px (was competing with content)
- **Stat numbers**: added `font-weight:700` and proper `line-height:1` for cleaner number/label hierarchy

### Added — #5 First-Run Experience
- Day 1 hero card shows SDP baseline target instead of `— × —`
- Dashboard empty state card: contextual message when no sessions logged yet
- Pre-populated weights from program baselines on first session (no blank fields)

### Added — #4 Coach Tab
- Analytics nav item renamed "Coach" with new flame icon
- Dedicated Coach panel (default view) with three sections:
  - **Weekly Readiness**: session count ring vs expected 4/week, total volume, avg sleep, stall count
  - **Today's Focus**: live SDP targets for every exercise in today's program, colour-coded for weight bumps (green) and first sessions (accent)
  - **Stall Flags**: active stalls from the detection engine, each with its top recommended action
- **Weekly Brief AI CTA**: calls `getCoachAI()` when API key is present; shows structured STATUS / PRIORITY / 3-action output
- Stats sub-tab: existing analytics charts moved behind a tab pill — lazy-loaded only when selected (avoids rendering Chart.js into hidden elements)
- `switchCoachTab(tab)` controls panel visibility and pill button active state

### Added — #7 Interactive Charts
- **Date range filter**: 7d / 30d / 90d / All pill row at top of Stats panel. `setRange(r)` updates `curRange` and re-renders strength and recomp charts
- **Strength chart range**: filtered view switches to date-based x-axis; full view retains 26-week projection lane
- **Recomp chart range**: squat history and waist log both filtered by cutoff date
- **Touch tooltips**: all charts now include `events: ['click','mousemove','touchstart','touchmove']` and `interaction: {mode:'nearest',axis:'x',intersect:false}` — tooltips work correctly on mobile touch
- `renderLiftChart()` extracted from `renderAnalytics()` as standalone function — called by lift tabs, range filter, and `renderAnalytics()`

### Added — #10 Calibrated AI Prompt
- `COACH_PROMPT_V1` versioned constant object with two structured builders:
  - `buildWeeklyPrompt(ctx)` — used by Coach tab Weekly Brief. Includes athlete context, weight change, sleep avg, session count, stall list, hex scores. Output format: STATUS / PRIORITY / 1-2-3 actions
  - `buildAxisPrompt(ctx)` — used by radar axis coaching (replaces the old inline prompt string in `getAICoaching()`)
- `getCoachAI()` async function — powers the Coach tab AI brief, handles loading state, parses and renders structured output
- System prompt tightened: "No filler. No emoji. Numbers where they add value."

### Changed — Navigation
- Analytics → Coach in bottom nav (icon updated to flame SVG)
- Page title: "ANALYTICS" → "COACH" with subtitle "Your personal training intelligence"
...
### Status
Project wound down intentionally at v3.1. Cloud sync, wearables, and multi-profile features were scoped and designed (see roadmap) but deliberately excluded from this portfolio project. The work achieved its objectives. Reopened at v4.0 for design refresh.
