...
[![Version](https://img.shields.io/badge/version-4.0-f5c842)](#)
...
### 🧠 Coach (v4.0)
- **Weekly Readiness card** — session count vs target, total volume, avg sleep, active stalls at a glance
- **Today's Focus** — live SDP targets for every exercise in today's session, colour-coded for weight bumps
- **Stall Flags** — active plateau/fatigue detections surfaced with top recommended action each
- **Weekly Brief** — AI-generated STATUS / PRIORITY / 3-action brief from your actual week's data
- **Stats sub-tab** — full analytics suite behind a tab toggle (lazy-loaded)

### 📊 Analytics
- **Training composition radar** — 7-axis score with interactive coaching recommendations
- **Planned vs actual strength charts** — science-backed logarithmic progression curves with date range filter (7d / 30d / 90d / All)
- **Stall detection engine** — distinguishes Type 1 Plateau from Type 2 Fatigue, with different science-backed recommendations
- **Grouped exercise browser** — all 29 exercises organised by muscle group with trend indicators
- **Weekly volume tracker** and **recomp dual-axis chart** — both filterable by date range
- **Touch tooltips** — tap any chart point on mobile to see exact values
...
- **Weekly Brief** (v4.0) — structured prompt ingests your week: sessions, stalls, weight change, sleep, hex scores → STATUS / PRIORITY / 3 actions
- **Axis coaching** — tap any radar axis for coaching personalised to your actual data
- Post-workout analysis: session recap, next week targets with exact weights, single focus point
- Versioned prompt template (`COACH_PROMPT_V1`) — structured, reproducible, upgradeable
...
Zero budget + no coding background + personal use = the simplest architecture that works. Client-side persistence via localStorage means no server costs, no privacy concerns, no account management. The entire app ships as a single HTML file (~251KB at v4.0).

---

## Quick Start

1. Download `gainr/gainr.html`
...
| v3.1 | Profile page — identity card, inline editing, goal management | ✅ Released |
| v4.0 | Design refresh (OKLCH), Coach tab, interactive charts, calibrated AI prompt | ✅ Released |
| v5.0+ | Multi-profile, cloud sync, wearables — designed, not built (scope decision) | 🚫 Descoped |
